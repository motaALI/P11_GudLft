...
def check_club_possibility_to_reserve_places(club, placesRequired):
    ...
    if placesRequired > int(point_available):
        raise bookingError(
            f"You don't have enough points. Points remaining ({point_available})!"
        )
    elif placesRequired > maxAuthorathedToReserve:
        raise bookingError(
            f"You cannot reserve more than ({maxAuthorathedToReserve}) places!"
        )
    else:
        return True
...
def check_reservation_not_in_past(competition_date: datetime):
    try:
        if not is_competition_in_present(competition_date):
            raise bookingError(
                "You are not able to book a place on a post-dated competition."
            )
